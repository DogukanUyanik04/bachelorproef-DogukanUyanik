...
Hoewel de decompilatie succesvol was in het herstellen van de mappenstructuur, stuitte het onderzoek op een fundamentele barrière. Uit de decompilatie (zie Figuur \ref{fig:jadx_obfuscation}) bleek dat de applicatie is gebouwd op het cross-platform Xamarin/Mono framework (te zien aan de \texttt{IGCUserPeer} implementaties). De zichtbare Java-code fungeert uitsluitend als een \textit{wrapper} die de datastromen via \texttt{native} methoden direct doorstuurt naar onderliggende gecompileerde C\#-assemblies (.dll).
Hierdoor was het onmogelijk om de logica achter de Bluetooth-handshake of de data-interpretatie puur uit de Java-broncode af te leiden. Deze doodlopende weg bevestigde de noodzaak voor een \textit{black-box} benadering via netwerkanalyse op de hardware-interface.
...
gezocht naar \textit{GATT (Generic Attribute Profile)} transacties. Voorafgaand aan de analyse werden de specifieke GATT-services in kaart gebracht via nRF Connect (zie Figuur \ref{fig:nrf_uuids}).
...
Een cruciale ontdekking was dat de pop beveiligd is met een authenticatie-mechanisme. Zonder het versturen van een specifieke \textit{Auth Key} naar de pop, weigert de hardware sensordata te verzenden. De geanalyseerde handshake bestaat uit drie stappen:
\begin{enumerate}
...
Na de succesvolle handshake begint de pop met het uitzenden van \textit{notifications} op het \textit{Wave}-kanaal (zie Figuur \ref{fig:wireshark_wave}). De data wordt verzonden als een byte-array. 
...
De ruwe waarde in deze bytes correleert direct met de fysieke indrukking van de borstkas. In de Unity-omgeving wordt deze waarde genormaliseerd en vertaald naar een procentuele diepte voor de visuele feedback-elementen in de Mixed Reality-ervaring.
...
\textit{packet sniffing} en het handmatig implementeren van de handshake in C\#, is een stabiele databrug geslagen tussen de medische hardware en de Meta Quest 3.  TESTEESTTESTTESTTEST